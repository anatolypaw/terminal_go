...
func main() {
	...
	for {
		f.Read(b)
		sec := binary.LittleEndian.Uint16(b[0:4])
		usec := binary.LittleEndian.Uint16(b[4:8])

		var value int16
		binary.Read(bytes.NewReader(b[12:14]), binary.LittleEndian, &value)

		event := inputEvent{
			time:  time.Unix(int64(sec), int64(usec)),
			typee: uint8(binary.LittleEndian.Uint16(b[8:10])),
			code:  uint8(binary.LittleEndian.Uint16(b[10:12])),
			value: int(value),
		}
		...
		if x_done && y_done && event.typee == TYPE_PRESS && event.value == 1 {
			fb.Clear()
			draw.Draw(display, display.Bounds(), img, image.Point{x, y}, draw.Over)
			x_done = false
			y_done = false
		}
	...
	/*
		for {
			...
			for i := 0; i < 800; i += 2 {
				draw.Draw(fb, imgb, img, image.Point{i - 800, int(math.Sin(float64(i)/10)*8) - 120}, draw.Src)
			}

			img = imaging.FlipH(img)
			for i := 800; i > 0; i -= 4 {
				draw.Draw(fb, imgb, img, image.Point{i - 800, int(math.Sin(float64(i)/20)*16) - 120}, draw.Src)
			}
